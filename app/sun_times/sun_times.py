...
class SunDisplayer:
    def __init__(self, lat, lon, now: datetime = datetime.today()):
        ...
        self.font = ImageFont.truetype(Path(__file__).parent / "assets/fonts/04B_03__.TTF", size=8)
    ...
    def _drawText(self, draw: ImageDraw.ImageDraw):
        ...
        time_str = self.today.strftime("%H:%M")
        bbox = draw.textbbox((0, 0), time_str, font=self.font)
        x = (32 - (bbox[2] - bbox[0])) // 2 - bbox[0]
        y = (32 - (bbox[3] - bbox[1])) // 2 - bbox[1] + 1
        draw.text((x, y), time_str, fill=None, font=self.font)
    ...
    @staticmethod
    def _arc_point(dt: datetime) -> float:
        return (dt.hour + dt.minute / 60) / 24 * 360 + 90

    def _drawArcs(self, draw: ImageDraw.ImageDraw):
        arc_w: int = 2
        draw.arc(
            ((1, 1), (30, 30)),
            SunDisplayer._arc_point(self.todayData.sunset),
            SunDisplayer._arc_point(self.todayData.sunrise),
            fill="#7a7a7a",
            width=arc_w,
        )  # night arc
        draw.arc(
            ((1, 1), (30, 30)),
            SunDisplayer._arc_point(self.todayData.dawn),
            SunDisplayer._arc_point(self.todayData.dusk),
            fill="#07446e",
            width=arc_w,
        )  # light arc
        draw.arc(
...
def getSunData(lat: float, lon: float, date: datetime = datetime.today()) -> SunData:
    ...
    day_len: int = (
        int(results["day_length"].split(":")[0]) * 3600
        + int(results["day_length"].split(":")[1]) * 60
        + int(results["day_length"].split(":")[2])
    )

    data = SunData(
        date=data_date,
        sunrise=datetime.strptime(
            f"{results['date']} {results['sunrise']}", "%Y-%m-%d %I:%M:%S %p"
        ),
        sunset=datetime.strptime(f"{results['date']} {results['sunset']}", "%Y-%m-%d %I:%M:%S %p"),
        first_light=datetime.strptime(
            f"{results['date']} {results['first_light']}", "%Y-%m-%d %I:%M:%S %p"
        ),
        last_light=datetime.strptime(
            f"{results['date']} {results['last_light']}", "%Y-%m-%d %I:%M:%S %p"
        ),
        ...
        golden_hour=datetime.strptime(
            f"{results['date']} {results['golden_hour']}", "%Y-%m-%d %I:%M:%S %p"
        ),
        day_length=day_len,
    )
    return data
